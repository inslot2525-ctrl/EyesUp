...
DEST_PRIMARY="/sdcard/Pillion/config"
DEST_FALLBACK="/sdcard/Android/data/com.pillion.app/files/config"
...
if adb $DEVICE_ARG shell "mkdir -p $DEST_PRIMARY" 2>/dev/null; then
  ...
  echo "    /sdcard/Pillion not writable (scoped storage) - falling back"
...
echo "==> done. Press 'Reload config' in Pillion Settings to apply without a restart."
