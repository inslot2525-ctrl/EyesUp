...
DEST_PRIMARY="/sdcard/Sarthi/config"
DEST_FALLBACK="/sdcard/Android/data/com.sarthi.copilot/files/config"
...
if adb $DEVICE_ARG shell "mkdir -p $DEST_PRIMARY" 2>/dev/null; then
  ...
  echo "    /sdcard/Sarthi not writable (scoped storage) - falling back"
...
echo "==> done. Press 'Reload config' in Sarthi Settings to apply without a restart."
