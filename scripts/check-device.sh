...
echo
echo "=== NOTIFICATION LISTENER (is EyesUp enabled?) ==="
...
adb $DEVICE_ARG shell "dumpsys deviceidle whitelist | grep -i eyesup" || echo "  not whitelisted - fix this, see DEVICE_AND_TOOLING_SETUP.md section 4"

echo
echo "=== STORAGE WRITE TEST ==="
adb $DEVICE_ARG shell "mkdir -p /sdcard/EyesUp/config && echo ok > /sdcard/EyesUp/config/.probe && cat /sdcard/EyesUp/config/.probe" \
  || echo "  /sdcard/EyesUp not writable - use the app-private external dir instead"
