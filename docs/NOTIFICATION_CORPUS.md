...
> `SarthiNotificationListener` resolves the app from the channel id first. Package matching only
...
1. At 0.0 s Sarthi speaks a verdict on S1 alone (a marginal TAKE at market rate).
2. At 2.5 s R1 arrives, scores higher, and Sarthi speaks the **arbitration line**:
...
3. At 5.0 s U3 arrives, is beaten by the still-live R1, and is skipped with
   `BEATEN_BY_OTHER_OFFER` + `LONG_DEADHEAD`. Sarthi does not re-announce a winner it already named.
